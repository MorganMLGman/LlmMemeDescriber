...
def test_calculate_phash_returns_hex_for_rgb_image(load_test_image):
...
def test_calculate_phash_handles_alpha_channel(load_test_image):
    data = load_test_image("rgba.png")
    phash = calculate_phash(data)
    assert phash is not None


def test_calculate_phash_handles_paletted_images(load_test_image):
    phash = calculate_phash(load_test_image("paletted.png"))
    assert phash is not None


def test_calculate_phash_is_same_for_identical_images(load_test_image):
...
def test_calculate_phash_differs_for_different_images(load_test_image):
    data1 = load_test_image("rgb.png")
...
def test_calculate_phash_handles_grayscale_image(load_test_image):
    data = load_test_image("grayscale.png")
    phash = calculate_phash(data)
    assert phash is not None


def test_calculate_phash_handles_image_formats(load_test_image):
...
def test_hamming_distance_zero_for_identical_hashes(load_test_image):
    phash = calculate_phash(load_test_image("rgb.png"))
    assert phash is not None
    assert hamming_distance(phash, phash) == 0


def test_hamming_distance_positive_for_different_images(load_test_image):
...
def test_hamming_distance_is_symmetric(load_test_image):
    phash1 = calculate_phash(load_test_image("rgb.png"))
    phash2 = calculate_phash(load_test_image("rgb_variant2.png"))
    assert phash1 is not None and phash2 is not None
...
def test_find_duplicate_groups_detects_similar_memes(in_memory_session):
...
def test_find_duplicate_groups_transitive_not_merged(in_memory_session):
    T = DUPLICATE_THRESHOLD
    session = in_memory_session
    a_ph = hex_ones(0)
...
def test_find_duplicate_groups_multiple_groups(in_memory_session):
    T = DUPLICATE_THRESHOLD
    session = in_memory_session
    # group1: low bits
...
def test_find_duplicate_groups_pair_exception_with_alternate_partner(in_memory_session):
    T = DUPLICATE_THRESHOLD
    session = in_memory_session
...
def test_find_duplicate_groups_includes_false_positive_flagged_memes(in_memory_session):
    session = in_memory_session
...
def test_find_duplicate_groups_no_duplicates(in_memory_session):
    session = in_memory_session
...
def test_find_duplicate_groups_empty_db(in_memory_session):
    session = in_memory_session
    groups = find_duplicate_groups(session)
    assert len(groups) == 0


def test_find_duplicate_groups_single_meme(in_memory_session):
    session = in_memory_session
...
def test_find_duplicate_groups_no_phashes(in_memory_session):
    session = in_memory_session
...
def test_find_duplicate_groups_ignores_memes_without_phash(in_memory_session):
    T = DUPLICATE_THRESHOLD
    session = in_memory_session
...
def test_mark_false_positive_returns_false_for_missing(in_memory_session):
    session = in_memory_session
    assert mark_false_positive(session, "no_such_file.png") is False


def test_mark_false_positive_sets_flag_and_removes_links(in_memory_session):
    session = in_memory_session
...
def test_mark_false_positive_handles_link_removal_exception(monkeypatch, caplog_set_level, in_memory_session):
    caplog_set_level(logging.DEBUG, logger='llm_memedescriber.deduplication')
    session = in_memory_session
...
def test_merge_duplicates_no_duplicates_returns_false(in_memory_session):
    session = in_memory_session
...
def test_merge_duplicates_merges_metadata_and_deletes(in_memory_session):
    session = in_memory_session
...
def test_merge_duplicates_respects_metadata_sources(in_memory_session):
    session = in_memory_session
...
def test_merge_duplicates_storage_delete_exception_is_logged(caplog_set_level, caplog, in_memory_session):
    caplog_set_level(logging.WARNING)
    session = in_memory_session
...
def test_merge_duplicates_cleans_up_groups(in_memory_session):
    session = in_memory_session
    g = DuplicateGroup()
    session.add(g)
    session.commit()
...
def test_merge_duplicates_rolls_back_on_exception(monkeypatch, in_memory_session):
    session = in_memory_session
...
def test_list_pair_exceptions_empty(in_memory_session):
    session = in_memory_session
    res = list_pair_exceptions(session)
    assert isinstance(res, list)
    assert res == []

def test_list_pair_exceptions_returns_all(in_memory_session):
    session = in_memory_session
...
def test_add_pair_exception_creates_new_duplicate(in_memory_session):
    session = in_memory_session
...
def test_add_pair_exception_updates_existing_and_is_idempotent(in_memory_session):
    session = in_memory_session
...
def test_remove_pair_exception_returns_false_when_missing(in_memory_session):
    session = in_memory_session
