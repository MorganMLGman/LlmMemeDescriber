...
def test_get_group_members_and_groups_for_filename_empty():
...
def test_set_group_links_and_getters():
    with create_in_memory_session() as session:
...
def test_clear_group_links_for_filename_removes_only_target():
    with create_in_memory_session() as session:
...
def test_set_group_links_allows_duplicate_entries_and_clear_handles_none():
    with create_in_memory_session() as session:
...
def test_get_group_members_raises_on_non_int():
    with create_in_memory_session() as session:
        with pytest.raises(TypeError):
            get_group_members(session, "not-an-int")


def test_get_groups_for_filename_raises_on_non_str():
    with create_in_memory_session() as session:
        with pytest.raises(TypeError):
            get_groups_for_filename(session, None)


def test_set_group_links_raises_on_non_list_or_non_str_members():
    with create_in_memory_session() as session:
        with pytest.raises(TypeError):
...
def test_clear_group_links_raises_on_non_str_filename():
    with create_in_memory_session() as session:
        with pytest.raises(TypeError):
...
def test_set_group_links_with_empty_list_is_noop():
    with create_in_memory_session() as session:
...
def test_get_group_members_and_groups_with_multiple_groups():
    with create_in_memory_session() as session:
...
def test_set_group_links_inserts_duplicate_rows():
    with create_in_memory_session() as session:
...
def test_clear_group_links_noop_when_missing():
    with create_in_memory_session() as session:
